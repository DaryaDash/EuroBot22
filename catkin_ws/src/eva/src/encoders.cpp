...
EncodersPair::EncodersPair(double update_rate) :
	_encoder_left(17, 27, &EncoderWiringPiISR::encoderISR1, &EncoderWiringPiISR::encoder_position_1),
    _encoder_back(2, 1, &EncoderWiringPiISR::encoderISR3, &EncoderWiringPiISR::encoder_position_3),
	_encoder_right(24, 25, &EncoderWiringPiISR::encoderISR2, &EncoderWiringPiISR::encoder_position_2) {
    ...
    _back_wheel_angle_pub = _node.advertise<std_msgs::Float64>("/abot/back_wheel/angle", 1);
    ...
    _back_wheel_velocity_pub = _node.advertise<std_msgs::Float64>("/abot/back_wheel/current_velocity", 1);
...
void EncodersPair::encodersCallback(const ros::TimerEvent& event) {
    ...
    double delta_back_wheel = _back_wheel_angle - _back_wheel_position;
    ...
    _back_wheel_position += delta_back_wheel;
	_back_wheel_velocity = delta_back_wheel / elapsed.toSec();
    ...
    _back_wheel_velocity_pub.publish(_back_wheel_velocity_msg);
}
